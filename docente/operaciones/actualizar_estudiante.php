...
if(($dni_a <> $dni)){
    ...
    $sql = "UPDATE estudiante SET apellidos_nombres='$nom_ap', id_genero='$genero', fecha_nac='$fecha_nac', direccion='$direccion', correo='$email', telefono='$telefono', anio_ingreso='$anio_ingreso', id_programa_estudios='$carrera', id_semestre='$semestre', seccion='$seccion', turno='$turno', discapacidad='$discapacidad' WHERE id=$id";
    $ejec_consulta = mysqli_query($conexion, $sql);
    if ($ejec_consulta) {
        if($egresado == "NO"){
            echo "<script>
